...
class ObjectLocationTracker():
    ...
    def __init__(self):
        self.object_dict = {}
        self.stop_list = self.getStopList()
        ...
        # Publishers
        self.obj_pub = rospy.Publisher('/stopLocations', ObjectLocations, queue_size=10)

    @classmethod
    def getStopList(cls):
        """ Return list of labels to identify as foods """
        path = '/'.join(os.path.realpath(__file__).split('/')[0:-1])
        with open(path+'/list_stop') as f:
            lines = f.read()
            stops = lines.split()
        return stops
